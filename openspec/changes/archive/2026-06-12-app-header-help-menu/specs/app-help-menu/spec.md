...
### Requirement: Desktop FroggersTiga menu exposes help docs

The desktop sim SHALL provide a **FroggersTiga** application menu (macOS menu bar) or **Help** menu (Windows/Linux) with items **Manual**, **Quick Dict**, and **License**. Selecting an item SHALL open a read-only scrollable dialog showing the bundled document text without requiring network access.

#### Scenario: Open Manual on macOS

- **WHEN** the user chooses **Manual** from the **FroggersTiga** menu
...
### Requirement: Web header opens help menu

The web sim header SHALL treat the **FroggersTiga** title cluster (logo + heading) as a menu trigger. Activating it SHALL reveal a menu with **Manual**, **Quick Dict**, and **License**. Selecting an entry SHALL open a modal overlay with the document text without navigating away from the sim page.

#### Scenario: Header click shows menu

- **WHEN** the user clicks the FroggersTiga header trigger
- **THEN** a menu with three items appears below the header
