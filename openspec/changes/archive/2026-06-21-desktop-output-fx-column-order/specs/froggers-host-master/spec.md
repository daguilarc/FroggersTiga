...
Host page **indices and labels** remain `2=Reverb`, `3=Filter`, `4=Drive` in `ParamDisplayNames`. Desktop standalone MAY permute **horizontal column positions** of pages 2, 3, and 4 so columns read Drive → Filter → Reverb left-to-right, matching `FroggersEngine` output FX order (`FrogBlock` then filter stages then reverb wet/dry), without renumbering pages.
...
- **THEN** Drive (page 4), Filter (page 3), and Reverb (page 2) MAY appear left-to-right in signal order while host page indices and cross-host labels stay unchanged
