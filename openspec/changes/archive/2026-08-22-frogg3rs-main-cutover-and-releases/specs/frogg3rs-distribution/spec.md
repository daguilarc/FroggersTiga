...
**Added 2026-08-21.** A new capability. Nothing previously stated what the
published site, the desktop download and the plugin download are, so each drifted
independently: the site serves a build from a renamed repository under a retired
product name, the desktop release builds a frozen tree, and the plugin has no
release at all.
