...
- **THEN** a dialog opens titled **Manual** with the full `SIM_MANUAL.md` content scrollable inside the app
...
- **THEN** a modal displays `sim-manual.md` content fetched from the site static assets
...
Build or sync scripts SHALL copy `SIM_MANUAL.md`, `QUICK_DICT.md`, and `LICENSE` from the repository root into desktop `BinaryData` and web static assets so desktop and web show the same canonical sim help text. Firmware `MANUAL.md` SHALL NOT be copied into sim host assets.

#### Scenario: Web build includes docs

- **WHEN** `npm run build` completes for the web sim
- **THEN** `public/sim-manual.md`, `public/quick-dict.md`, and `public/license.md` exist and match repo root sources at build time
- **AND** `public/manual.md` is not produced by the sync script
