...
- [ ] 1. Add `paths-ignore: ['openspec/**', '**.md']` under `push:` in
  `pages.yml` and under both `push:` and `pull_request:` in
  `vst-plugin.yml`. Touch nothing else in either file.
- [ ] 2. Commit and push (this push changes workflow files, so both builds
  SHOULD run — that is the positive control's first half).
