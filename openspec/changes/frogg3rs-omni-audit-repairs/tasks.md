...
- [ ] 2.1 Root `Makefile`: target `firmware-test` (added to `.PHONY`) that
      ...
      `&&` chain.
- [ ] 2.2 `.github/workflows/firmware-tests.yml`: `ubuntu-latest`, checkout
...
- [ ] 2.3 Gate: `make firmware-test` locally, 5/5 pass. Record the pass line.
- [ ] 2.4 `README.md`: one sentence under the firmware section naming
...
- [ ] 3.3 `app/FroggersUiSurface.hpp:188-194` and `:1489-1495`: state that
...
- [ ] 3.4 Enumerate (H) every `<Name>.(hpp|cpp):<n>` citation in
      ...
      as before → after.
- [ ] 3.5 (4a) `stereo_delay_clear_buffers_resets_to_silence`: feed 4800
...
- [ ] 3.6 (4b) `envelope_followers_track_abs_value_with_attack_release_asymmetry`:
      `expectedFall = ef.attackCoeff * (1.0f - ef.releaseCoeff)`;
      `REQUIRE_NEAR(out[0], expectedFall, 1e-6)`; the `(void)` line and the
      `<` assertion go.
- [ ] 3.7 Gate: `make -C app test` and each test binary by path. 3.5 and 3.6
      must pass; record the parity binary's count line.

## 4. Structure in `app/` (findings 6, 7) — S

- [ ] 4.0 Positive control, before any edit: in `RouteAudioSample()` negate
...
- [ ] 4.1 `FroggersUiSurface.hpp`: private
...
- [ ] 4.5 Dropped (P2).
- [ ] 4.6 Remove the `:669-671` guard; `drillIn_` is emplaced in `Init()`
      (`:304`). Remove the `:1297-1299` guard and its three-line comment; the
      `drillLevelDisplay_` store reads `drillIn_->Level()` directly.
- [ ] 4.7 Gate: `make -C app test` and each test binary by path; then
...
- [ ] 7.1 `.gitignore`: delete `:2-8` (six trees that no longer exist; keep
      ...
      one line).
- [ ] 7.3 The six `app/` comments drop `desktop-v2` (and the parity file's
...
- [ ] 7.4 The spec delta in this change removes
...
- [ ] 8.2 `openspec/changes/frogg3rs-controllers-page-user-story/proposal.md`:
      ...
      --skip-specs -y`.
- [ ] 8.3 `openspec/changes/frogg3rs-guitar-and-solo-variants/specs/external-ring-mod-mix/spec.md`:
...
- [ ] 6.1 (H) Enumerate every line in `app/*.hpp`, `app/*.cpp`, `app/dsp`,
...
- [ ] 6.2 (S) Classify every hit: label (a planning identifier the reader
...
- [ ] 6.3a (S) Edit each label line under `app/`: the label goes, the
...
- [ ] 6.4a Gate: `make -C app test` and each test binary by path; `nice
...
- [ ] P.1 Commits so far: change dir; 2; 3; 4; 7a; 8a; 6a. Push the branch,
...
- [ ] 5.1 (H) Delete `src/common/Comb.hpp`, `Marbles.hpp`, `ModMgr.hpp`,
...
- [ ] 5.3 (H) `git rm External/theallelectricsmartgrid`; delete its
...
- [ ] 5.4 (H) `git rm -r publish/`. (S) Rewrite
...
- [ ] 5.4b (S) Move `RefreshGate` from `src/common/DaisyIO.hpp:15-32` to
...
- [ ] 5.7 (H enumerates, S resolves) Every `app/` citation into a `src/core`
...
- [ ] 6.1b (H) Re-run 6.1 over `src/core`, `src/common`, `src/Froggers*`,
      `src/mk`, `test/firmware` after group 5. (S) Classify as 6.2.
- [ ] 6.3b (S) Edit each label line as 6.3a.
- [ ] 6.4b Gate: `make firmware-test`; `git diff -w` read hunk by hunk: no
      non-comment line changed in this commit.

## 7b, 8b. desktop-v2 and docs, local-only part

- [ ] 7.2 `src/core/FroggersEngine.hpp`: any surviving comment naming
...
- [ ] 8.1c `DAISY_MANUAL.md:15`: the "Frozen" sentence becomes: the firmware
...
- [ ] 9.1 `test/firmware/Parameter_fuego_test.cpp`: knob 0 → output equals
...
- [ ] 9.2 `DspModules_test.cpp`: one property each for `BiquadSection`,
...
- [ ] 9.3 `Page_test.cpp`: `KnobUpdate` moves a parameter, mod source and
...
- [ ] 9.4 Gate: `make firmware-test`, 8/8 pass. Each new binary is run
...
- [ ] 10.1 Every gate in the proposal's table, run again on the final tree,
      pass lines recorded next to the first-run lines from 1.3.
- [ ] 10.2 §5 against the diff (S, fresh context): every new helper
...
- [ ] 10.3 Postflight (S, fresh context): implementation vs this text, per
      task, divergence reported strictly; the spec deltas checked against a
      passing check each.
- [ ] 10.4 Local-only commits listed by hash in the ledger; `git status`
