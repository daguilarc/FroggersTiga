# desktop-v2-mod-source-grid Specification

## Purpose
Desktop v2 modulation source assignment, parameter-detail depth editing, and Rand Mods derive route eligibility from the Froggers v2 app manifest and permanent 15-lane source catalog.
...
Desktop v2 parameter-detail modulation view SHALL render sixteen encoder cells at the default standalone size: fifteen source-depth encoders plus one dedicated **Target (Back)** encoder. Each source-depth cell SHALL display a display-only CV activity underlay from the shared fifteen-lane CV history for that lane. The Target (Back) cell SHALL NOT show an underlay. The Target (Back) encoder SHALL remain visible while source depths are edited. Encoder interaction SHALL remain: ring annulus → `ParamTurn`; center MOD → `ModDrillIn`; Target press → exit detail.
...
- **THEN** one cell corresponds to the dedicated Target (Back) encoder
- **THEN** no source lane is hidden behind scrolling at the default standalone size

#### Scenario: Target remains available during depth editing
- **WHEN** the user is editing source depths for a target parameter
- **THEN** the dedicated Target (Back) encoder remains visible
