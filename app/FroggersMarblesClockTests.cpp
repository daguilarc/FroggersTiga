// FroggersMarblesClockTests.cpp -- tasks.md section "8. Marbles: clock +
// visualizer" (design D8/D8a), tasks 8.4/8.5. Bare synth::ParameterManager +
...
// the per-source clock/rate logic (task 8.1) is fully exercisable without a
// real MasterClock/AudioBlock/SynthRig. Task 8.4's "high-deja-vu source
...
// DSP's own parity suite. Task 8.6 ("validate D8a's character table by
// ear") is NOT covered here -- see this packet's own report.
...
// -----------------------------------------------------------------------
// Task 8.4 -- "over N quarter notes each source advances exactly N x its
// own rate multiple (#1, #4 -> N; #2 -> 2N; #3 -> 3N; #5 -> N/4)." Since
// every one of the five lanes is constructed with stepChance=1.0 (task
// 3.4's "all five are constructed at 1.0 (always step)") and RandomShLane::
...
// counted in ticks." PrepareBlockClock() (task 8.1) recomputes source #6's
...
TEST_CASE(source_six_tempo_following_input_scales_inversely_with_quarter_notes_per_sample) {
    ...
    // positive, valid config -- task 8.1's "clamp finite and positive
...
// Task 8.3: source #6 reuses GangedRandomLfoVisualizer UNMODIFIED, while the
...
// Task 8.4: "visualizer state matches the bag" -- RandomShLaneVisualizer
// reads dsp::RandomShLane::UiState directly (task 8.3), so after driving a
...
TEST_CASE(visualizer_state_matches_the_bag_after_publishing) {
    ...
    // (task 6's own "source-value convention" -- see FroggersModulation.hpp's
    // file header), not left at some uninitialized/out-of-range default.
...
// -----------------------------------------------------------------------
// Task 8.5 -- "Confirm no source-level control exists: no Random S&H
// behaviour parameter in any bank, no seventh bank (D8)."
// -----------------------------------------------------------------------
...
// GangedRandomLfoProcessor<1> instance (FroggersModulation.hpp:611, D8's
// resolution of source #6, NOT one of StandardModulators' processors), so
...
// (FroggersModulation.hpp:517/489-491, same formula as lanes 1-5). The
