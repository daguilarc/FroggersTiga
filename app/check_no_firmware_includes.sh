#!/usr/bin/env bash
# check_no_firmware_includes.sh -- a mechanical
# check, like the existing include guards, that no app TU includes from
# the firmware tree (src/): the DSP port is a sanctioned COPY,
...
# without a leading ../).
#
# Usage: check_no_firmware_includes.sh <app-dir>
...
while IFS= read -r -d '' file; do
  if grep -inE "$frozen_pattern" "$file" > /dev/null; then
    echo "check_no_firmware_includes: FAIL - $file includes from the firmware tree:" >&2
...
if [ "$status" -eq 0 ]; then
  echo "check_no_firmware_includes: OK - no app/** file includes from src/"
fi
exit "$status"
